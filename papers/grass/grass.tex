...
sequentially, and is paid the value at the final point. This is a
terminal placement problem, distinct from approximating the path's
maximum. The
...
the constant factor $(1+\rho)/(1-\rho)=e^{2\Theta}$, where $\Theta$
is the bracket rapidity. This matched-mean dominance extends to every
...
anchor takes over. Above that ceiling the grass is not greener, since
mean reversion caps the conditional mean of every unvisited point and
no variance bonus compensates. Numerically, the interior option is worth
...
final location. In between, the searcher steps to the distance at which the
