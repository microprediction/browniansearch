...
\begin{abstract}
\noindent How should a firm spend its last pivot? In field experiments most
startups pivot at most once or twice before committing. We solve
this scarce-trial search problem exactly. A searcher knows the
performance of an incumbent strategy, may run one more trial, and
must then commit. Performance is the exponential of a stationary
...
modeling, and fitness-landscape theory. The optimal policy is a
phase diagram with every boundary in closed form. A below-average
incumbent is abandoned. A disappointing trial sends the searcher out
...
value at the optimum, so most of the last pivot's worth is the
information it produces. We verify the correlation assumption on
77{,}040 WiFi measurements.
\end{abstract}
...
premium is first order in the bracket correlation.
...
$R^{2}=0.98$; the scripts are in the paper's repository. The same
...
The rules are short. Abandon a below-average incumbent for fresh
ground. If the trial disappoints, leave past the better of the two
positions; do not attempt to repair the bracket (Appendix~A). If
both positions are good, commit between them: at the midpoint for
moderate values, closer to the better-known position for high ones.
If the incumbent is exceptional, stay. The gain from placing the
final position cleverly is about one percent at the optimum, so most
of the value of the last pivot is the information it produces, and
the case for making the trial at all is much stronger than the case
for any particular refinement afterward.
