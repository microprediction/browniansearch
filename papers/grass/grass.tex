...
workhorse of that tradition is myopic --- each period's step
maximizes the current outcome --- and the few-shot farsighted case
is not a stylization: across randomized samples of startups, roughly
three quarters pivot zero times in a year, a fifth exactly once, and
only six to eleven percent more than once, with interventions that
improve search shifting firms toward exactly one pivot rather than
none or many (Camuffo et al.\ 2020, 2024). A searcher holding an
incumbent position, one further trial, and a final commitment --- at
most two pivots --- is therefore not a truncation of the empirically
relevant problem; it is the empirically relevant problem. Within
