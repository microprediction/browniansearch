...
\begin{abstract}
\noindent How should a firm spend its last pivot? In randomized trials most
startups pivot at most once or twice before committing. We study the
scarce-trial search problem: a searcher knows the performance of an
incumbent strategy, may run one more trial, and must then commit. We
solve the final placement exactly and reduce the opening trial to a
one-dimensional optimization. Performance is the exponential of a stationary
Ornstein--Uhlenbeck path: unexplored performance regresses to the
industry average, and correlation decays exponentially with
distance, a shape standard in radio engineering, Gaussian-process
modeling, and fitness-landscape theory. For equal observed values the
policy is a phase diagram with every boundary in closed form. A
below-median incumbent is abandoned. A disappointing trial sends the searcher out
past the better of her two positions. When both positions are good
she commits between them, at the midpoint for moderate values and
nearer the better-known position for higher ones, and above an exact
ceiling she stays put. The commitment between positions rests on a
constant-variance result: an interior position has the same expected
performance as some bolder alternative and $(1+\rho)/(1-\rho)$ times
its variance, where $\rho$ is the correlation between the two known
positions. The interior option adds about one percent of expected
value at the optimum in our calibration, so most of the last pivot's
worth is the information it produces. The rule also works as an
algorithm: used as the line search inside a derivative-free
optimizer, it beats golden-section and Brent inner loops on rough
and on high-dimensional physical objectives in a public benchmark,
and loses on needle landscapes and on genuinely smooth slices.
\end{abstract}
...
the scripts are in the paper's repository. The same
shape is standard in several other fields. In Gaussian-process
...
real objectives (Snoek, Larochelle and Adams 2012). Weinberger
(1990) measures the ruggedness of a fitness landscape by the
...
path scale matters. We calibrate $s=1$. For general $s$ every
comparison maximizes $\mu + s\nu/2$ with observations in
...
The rule wins in two regimes and loses in two. On collision and
...
21 of 24 seeds. On high-dimensional objectives it also wins even
when slices are smooth --- a sixteen-dimensional wind-farm layout on
21 or 22 of 24 seeds --- because its one- or two-evaluation lines
let the outer loop try several times more directions than an inner
loop that spends eight to ten evaluations per line. This last win is
budget-starved rather than general: it appears when the dimension
times a classical line search's cost exceeds the whole evaluation
budget, so a method with a fixed number of directions per iteration
cannot complete one pass. The rule works in this pairing with a
cheap-line outer loop, not as a drop-in replacement for the line
search inside an existing optimizer; substituted into Powell, whose
direction updates rely on genuine line minimization, the rule loses
to Brent. It loses in two regimes. On landscapes too rough to carry navigable
structure, broad sampling wins and placement theory has nothing to
exploit; on needle landscapes, where the value hides in a spike, the
fixed full-segment probes of golden-section search are the right
move. On genuinely smooth slices the classical loops are preferable
in principle, though among these physics objectives the smooth-slice
case is rarer than reduced-order surrogates suggest: the real
collision and ballistics simulators are rough. Across the 22 objectives the rule ranks
first or second for 9. As a standalone optimizer, rather than an inner loop, the two-shot
variant is mid-field against a panel of standard derivative-free
methods (mean rank near the middle of eleven across the 22
objectives). Its relative standing is best at the smallest budgets,
around forty evaluations, and slips as the budget grows and
population methods such as CMA-ES overtake it. That is the expected
place for a rule derived from a few-evaluation terminal-placement
model.
...
adaptation, not from the objectives resembling an OU path. This fits
the theory. A terminal-placement payoff rewards correlated trials,
...
One regularity organizes when the rule helps. Its advantage
concentrates where the evaluation budget is comparable to the number
of local minima a slice presents; when the minima greatly outnumber
the budget, spread wins. A pre-registered out-of-sample test bears
...
exists when the observed value is high. Mean reversion caps the conditional mean of every unvisited point.
...
formulas; the adaptive optimization it requires remains to be
analyzed. The same placement problem arises as the
line-search step inside derivative-free optimizers, where one-
...
identification holds for every real $b$. For $v < 0$ any strictly
interior point has $B > 0$, so $\mu < \lambda_2 b$. The revisit of
