...
Two further results temper the romance of the pivot. The option to
...
dominance requires $\rho>\left(b-1\right)/\left(b+1\right)$, so
...
at the final location, not the running maximum of the path. That distinguishes it from the optimization literature on Brownian
landscapes, where Grill, Valko and Munos approximate the maximum of a
...
rather than to solve an adaptive placement game. The problem instead
belongs to the correlated-learning
tradition that Callander introduced, in which each trial is a policy
actually enacted and the last choice is the one lived with. The
pivot-count evidence of the introduction makes the
incumbent-one-trial-commitment game, at most two pivots, the
empirically relevant problem rather than a truncation of it. Within
