...
rests on a constant-variance identity: every interior position has
...
observation window, 24\% pivoted exactly once, and 17\%
pivoted more than once (Camuffo et al.\ 2024; the shares in the
smaller 2020 trial are 74\%, 19\%, and 7\%). These are one-year
observation windows, and over a startup's life a budget of only a few
major pivots is a plausible order of magnitude.\footnote{A widely
...
number.} Training founders to search
more scientifically raised the frequency of a single pivot and
lowered the frequency of repeated pivots. Strategic search, in short,
is a small-sample problem, and the interesting version of it is
farsighted rather than reactive.
...
flight, and the interior option is then worthless; as the start
$b\to-\infty$ the optimal opening correlation tends to zero and the
three-shot log-value tends to
$\tfrac12+\log(1+1/\sqrt{2\pi})=0.8357\ldots$ Second, the
...
Across the 22 objectives \texttt{grass3} ranks first or second for
nine among the five inner loops, by median over seeds
(Table~\ref{tab:linesearch}). A separate
...
on which \texttt{grass3} beats the best rival. Across all 22
objectives \texttt{grass3} has mean rank $2.86/5$ and is first or
second on nine; \texttt{free\_kick} is a representative loss. As a
standalone optimizer the two-shot form \texttt{grass2U} has mean rank
$5.73/11$ against the eleven-method panel of
Section~\ref{sec:linesearch}.}
...
Scripts, seeds, the pre-registration, and per-objective results are
in the repository.
...
can lie below both anchors: at $b = d = 0.7$ and bracket correlation
