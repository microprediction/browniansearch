...
observation windows; over a startup's life a budget of only a few
...
extra value on average. The exponential is a production or revenue
mapping from log-performance to money, not a convex utility over a
fixed prize; the searcher is risk neutral in dollars.
...
Within that corner it is a rare exactly characterized multi-step
acquisition: the final placement is algebraic, quartic in the unequal
case, and the opening action reduces to one-dimensional quadrature and
optimization. A recurring finding in this literature is that
multi-step lookahead buys only modest gains over the myopic rule (Wu
and Frazier 2019). The roughly one-percent figure in Section~9 is a
different quantity: it isolates the incremental value of permitting
interior terminal recommendations relative to a restricted but still
farsighted policy.
