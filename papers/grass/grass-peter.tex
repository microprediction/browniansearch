...
The rule is itself an acquisition function, and it sits at a
specific place in the standard taxonomy (Shahriari et al.\ 2016). Acquisition functions
sort on two axes. One is myopic versus lookahead: expected
...
confidence bound bounds cumulative regret, whereas knowledge gradient
and entropy search score a finally reported point. Our rule is
lookahead and terminal-recommendation, the knowledge-gradient corner,
and it fills the cell that is otherwise empty, an acquisition that is
exact and closed-form for a small multi-step budget rather than
one-step or approximate. A recurring finding in this literature is that
...
\begin{corollary}[Below-median start]\label{cor:bneg}
For $d=b<0$ the interior option is worthless: the searcher flees to
$\zeta(b)=\tfrac12$, and as $b\to-\infty$ the optimal bracket
correlation tends to zero, so with three shots the limiting log-value
is $\tfrac12+\log(1+1/\sqrt{2\pi})=0.8357\ldots$ Together with
Theorem~\ref{thm:boundary} this settles the symmetric game for every
real anchor.
\end{corollary}

\noindent Throughout, $\rho\in(0,1)$: at $\rho=0$ the anchors are
independent and every position is a fresh draw, while $\rho=1$
collapses the bracket to a point, so both are degenerate limits rather
than cases of the phase diagram.
