...
\title{When the Grass Really Is Greener\\[4pt]
...
The proverb warns that the grass only looks greener elsewhere. The
analysis says when it truly is. Go forth on bad news: a disappointing
anchor is abandoned, a disappointing second value sends the search
outward past the better end. Stay between on good news: when both ends
...
natural conjecture from the corrected endgame is a two-phase $k$-shot
policy: step outward while values keep climbing, each new high
re-anchoring the next step, then refine inside once a high region is
bracketed. Settling it is a dynamic program over the posterior, cheap
