...
\begin{proof}
Differentiating, $f'(x)\,x^{3}\left(1+\rho\right)\left(1-\rho^{2}\right)$
is proportional to
$b\,x\left(x^{2}-\rho\right)\left(1-\rho\right)-\left(x^{4}-\rho^{2}\right)$,
and $x^{4}-\rho^{2}=\left(x^{2}-\rho\right)\left(x^{2}+\rho\right)$
gives \eqref{eq:factor}. The quadratic factor has real roots iff
$b\left(1-\rho\right)\ge 2\sqrt{\rho}$, with product $\rho$ (the
mirror symmetry $x\leftrightarrow\rho/x$) and
$x_{+}=1$ exactly when $b\left(1-\rho\right)+
\sqrt{b^{2}\left(1-\rho\right)^{2}-4\rho}=2$, i.e.
$b=\left(1+\rho\right)/\left(1-\rho\right)$. At the middle,
$f''(\sqrt{\rho}) = \frac{2b}{\sqrt{\rho}\left(1+\rho\right)}
- \frac{4}{1-\rho^{2}}$, negative precisely when
$b<2\sqrt{\rho}/\left(1-\rho\right)$; the hyperbolic form follows
from $\tanh\tau=\sqrt{\rho}$, whence
$2\sqrt{\rho}/\left(1-\rho\right)=2\tanh\tau\cosh^{2}\tau=\sinh 2\tau$.
\end{proof}
...
\begin{proof}
Lower edge: for $b\le 1$, $\zeta(b)=\tfrac12\left(b^{2}+1\right)$ and
$U_{\mathrm{mid}}>\zeta$ reduces to the stated quadratic with
discriminant $8\rho\left(1-\rho\right)$; since
$b_{-}<\sinh 2\tau$ for every $\rho$ (their ratio is
$\left(2-\sqrt{2\left(1-\rho\right)}\right)\left(1-\rho\right)
/\left(2\left(1+\rho\right)\right)<1$), the middle is the interior
optimum near the lower edge and the comparison is exact there.
Upper edge: expanding at the end, $x=1-\varepsilon$,
\begin{equation}
f\left(1-\varepsilon\right) \;=\; b
+ \varepsilon\left(1-\frac{b\left(1-\rho\right)}{1+\rho}\right)
+ O\!\left(\varepsilon^{2}\right),
\end{equation}
so for $1\le b<e^{2\Theta}$ a point just inside the end strictly
beats staying ($\zeta(b)=b$), while for $b>e^{2\Theta}$ the interior
holds no critical point besides the middle minimum
(\eqref{eq:offmid} has left the unit range), $f$ increases from the
middle to both ends, and the interior supremum is the end value
itself: staying is optimal. On the remaining stretch
$b\in\left(\min\{1,b_{+}\},1\right]$ at small $\rho$, where the
middle can lose to $\zeta$ while the off-middle pair wins, the
inequality $f\left(x_{+}\right)>\zeta(b)$ is verified numerically on
a dense grid (single crossing observed throughout); the two edges of
\eqref{eq:region} are exact as proved.
\end{proof}
